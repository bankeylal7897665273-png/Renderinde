...
# Yeh route hosting platform ke Health Check ke liye bahut zaroori hai
@app.route('/')
def home():
    return jsonify({"status": "Bot is alive and running!"})
...
def join_team():
    ...
    if loop is None:
        return jsonify({"status": "error", "message": "Bot is still starting up, please wait."})
...
async def MaiiiinE():
    ...
    Uid, Pw = '4741753966', '2AD9E8F4C674922146E174C702B5F49C65CD68C9C9D20BF084154A15EEAEC0C9'
...
if __name__ == '__main__':
    # Flask server ko sabse pehle alag thread mein start kar diya
    flask_thread = threading.Thread(target=run_flask, daemon=True)
    flask_thread.start()
    
    # Phir apna main bot ka async loop start hoga
